...
public:
    ...
    void Notify(TNotifyUI& msg)
    {
        if( msg.sType == _T("itemselect") ) {
            ...
            if( msg.pSender->GetName() == _T("menu_Pass") ) {
                if( m_pOwner ) m_pOwner->GetManager()->SendNotify(m_pOwner, _T("menu_Pass"), 0, 0, true);
			}
			else if (msg.pSender->GetName() == _T("menu_Refuse")) {
				if (m_pOwner) m_pOwner->GetManager()->SendNotify(m_pOwner, _T("menu_Refuse"), 0, 0, true);
			}
        }
    }
